    ...
    public class DBHelper
    {
        ...
        public static List<TASKS> ExecuteDataSet(string sql, Dictionary<string, object> dic)
        {
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                tasks.STARTTIME = Convert.ToDateTime(data.Rows[i]["STARTTIME"]);
                ...
                tasks.CREATETIME = Convert.ToDateTime(data.Rows[i]["CREATETIME"]);
                tasks.UPDATETIME = Convert.ToDateTime(data.Rows[i]["UPDATETIME"]);
